...
function renderRules(rules) {
  ...
  $("#rulesTable").innerHTML = rules.map((rule) => `
    <tr>
      ...
      <td>${escapeHtml(rule.giftTitle)}${rule.giftValue > 0 ? `<br><small>Worth \u20B9${rule.giftValue}</small>` : ""}<br><small>Variant: ${escapeHtml(rule.giftVariantId)}</small>${rule.giftProductId ? `<br><small>Product: ${escapeHtml(rule.giftProductId)}</small>` : ""}</td>
